...
import { Search, ChevronRight, Zap, Shield, Truck, Star, ArrowRight, CheckCircle, LogIn, UserPlus } from 'lucide-react';
...
export default function Home() {
  ...
  const handleCategorySelect = (categoryId, categoryName) => {
    // Always pass the slug (categoryId from diagram) directly so Shop can match subcategories
    ...
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
                ...
                {!user && (
                  <motion.div className="flex gap-3 mb-8" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    <Button
                      onClick={() => base44.auth.redirectToLogin()}
                      className="bg-white text-slate-900 hover:bg-slate-100 font-semibold gap-2 rounded-xl h-11 px-5"
                    >
                    ...
                    <Button
                      onClick={() => base44.auth.redirectToLogin()}
                      className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-semibold gap-2 rounded-xl h-11 px-5"
                    >
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25 }}>
                <div className="flex gap-2">
                  ...
                  <Button onClick={goToShop} className="h-14 px-7 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-bold rounded-xl shadow-lg shadow-amber-500/25">
                    Search
                  </Button>
                </div>
              </motion.div>
                ...
                <div className="relative bg-white/5 border border-white/10 rounded-3xl p-4 backdrop-blur-sm">
                  ...
                  {selectedCategory && (
                    <motion.div className="mt-3 flex justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <Button onClick={goToShop} className="bg-amber-500 hover:bg-amber-400 text-black font-semibold gap-2">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
            {popularMakes.map((make, i) => (
              ...
              >
                <img src={make.img} alt={make.name} className="w-full h-full object-cover brightness-50 group-hover:brightness-75 group-hover:scale-110 transition-all duration-300" />
                <div className="absolute inset-0 flex items-end justify-center pb-3">
                  <span className="text-white font-bold text-sm bg-black/40 backdrop-blur-sm px-3 py-1 rounded-full">{make.name}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredParts.map((part, i) => (
              <motion.div key={part.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
      ...
      <section className="py-20 relative overflow-hidden">
        <div className="absolute inset-0">
          <img src="https://images.unsplash.com/photo-1487754180451-c456f719a1fc?w=1600&h=600&fit=crop&q=80"
            ...
            <motion.div className="text-white" initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="flex flex-wrap gap-6 mt-5 text-sm text-slate-400">
                {['Free to join', 'Instant listings', 'Secure payments'].map(p => (
                  <span key={p} className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-amber-400" />{p}</span>
                ))}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <Link to={createPageUrl('SupplierRegister')}>
                <Button size="lg" className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-bold px-10 h-14 rounded-xl shadow-2xl shadow-amber-500/30 gap-2 text-base">
